...
**Related, separate finding from the same investigation — not yet fixed, reported to Tom, awaiting scope decision**: `client_external_id` for a group's job rows (`groupExternalId` in `maidsBuildPreview`) is resolved independently of `maidsMatchClient`'s own match result — from the group's first CSV row's own `customerid` column, falling back to a fabricated `name|street` key, rather than from the actually-matched existing client's real `external_id`. When a matched client's stored `external_id` differs from what a given import's group key resolves to, that client's *job* rows attach correctly to nothing — an orphaned `client_id` invisible everywhere except a direct SQL anti-join. Confirmed live: 8 orphaned job rows across 4 distinct client_ids (business-wide, out of 14,064 imported), unrelated to Hoffman/Kane. Small in scope, real, and a plausible candidate for what "silently skipped" would actually look like for a client who genuinely lost rows — worth its own pass if Tom wants it chased.

**Also confirmed, not a gap**: `import_runs.warnings_json` exists as a column but is **never populated** — the client-side `maidsBuildPreview` already computes a real `warnings` array (summary-row skips, empty-date skips, unmapped team codes, status anomalies) for the in-browser preview screen, but neither `import_maids_start` nor `import_maids_finish`'s RPC signatures accept a warnings parameter, so it's built and then discarded, never persisted. Matches Tom's "log every silent skip clearly on future imports" ask directly — the computation already exists, it just needs a wire to the DB.
