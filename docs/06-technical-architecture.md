...
## 8ap. Per-client + batch re-geocoding UI (PR #93, migration 094)

Two prior findings drove this: (1) Tom flagged two specific clients ("2 missing pins" on the Schedule Map) whose addresses had never been geocoded, and (2) the broader diagnosis found this was systemic — 241 of 563 active clients had no `lat`/`lng`, because the Maids CSV importer never geocodes new clients and a later sqft-enrichment pass discards its own geocode result instead of persisting it. Tom confirmed: build both a per-client force-refresh button and a batch flow for the whole missing-coords list.

**Per-client button**: every client card's action row (next to Activity/Edit/Cancel Client) now has a "📍 Re-geocode" button, visible on all cards (not just ones missing coords, so Tom can force a refresh on any address). `reGeocodeClient(clientId)` calls the existing `window.PentaGeocode.geocodeAddress()` (unchanged — same Nominatim→Photon chain used everywhere else), writes `lat`/`lng`/`geocode_status`/`geocoded_at` via `PentaClients.updateClient` (all four fields were already read+write mapped from Sprint 7-map/migration 018 — no facade changes needed), shows a `showToast` success/error message, and disables itself mid-flight (`_geocodingInFlight` Set) so a double-tap can't fire two concurrent geocodes for the same client.

**Batch flow**: reused the existing chip-drawer architecture (`special[]` array + `toggleChip` + the `activeFilters` Set + a predicate line in `applyFilters()`) rather than building a new segmented-control tab — a new `'missing_geocode'` chip filters on the literal spec Tom gave (`lat == null || lng == null`, not `geocode_status !== 'ok'`, so a client with a manually-set status but real coordinates never shows up here). A new `#geocode-batch-bar` (shown/hidden by `_renderGeocodeBatchBar()`, called from `applyFilters()` alongside the existing segment renderers) sits above the list only while that chip is active, showing a live count and a "Re-geocode all missing (N)" button plus a progress fill bar. `batchReGeocodeMissing()` snapshots the current `filtered` array (so it geocodes exactly what's on screen, honoring any other active filters/search) and works through it sequentially, each client independently try/catched so one failure never halts the batch — final toast reports `ok`/`failed`/`rate_limited` counts.

**Rate limiting — deviated from the literal spec on purpose.** Tom's spec suggested "~200ms" between calls; the batch loop instead paces at 1100ms, matching `PentaGeocode.batchBackfill()`'s own existing (if still unwired) precedent, whose comment cites Nominatim's documented 1 req/sec ToS. 200ms risks 429s (and a possible IP-level throttle/ban) against a free public geocoding service this app depends on everywhere, not just here — not a tradeoff worth making to shave a few seconds off a background batch job.

**Audit trail — new `geocode_history` table (migration 094)**: every call from either UI path (per-client or batch) writes one row — `business_id`, `client_id` (FK `clients.id`, i.e. the UUID form, not the legacy `external_id`), `called_at`, `called_by` (resolved from the auth session to `public.users.id` via the same `_resolveCallerUserId()` pattern `PentaJobIssues` already established, not the raw auth UUID — PR #88's `cancelled_by` column used the raw `session.user.id` directly against a `REFERENCES public.users(id)` FK, a likely id-shape mismatch this deliberately avoids), `address_at_time` (the address as of the call, independent of later edits), `result_lat`/`result_lng`/`result_status`. Append-only: SELECT + INSERT RLS policies only (no UPDATE/DELETE), mirroring `job_issues` (migration 047). New `window.PentaGeocodeHistory` module owns this — kept separate from `PentaGeocode` itself, which only resolves one address and has no Supabase/tenant/auth awareness. A logging failure is swallowed (never blocks the actual geocode UX, which has already completed and persisted by the time the log write runs).

Verified: 15 Node assertions (extraction-based against the real source — filter predicate logic, batch-bar wiring, try/catch resilience in the batch loop, the 1100ms pacing, per-client double-tap guard) all pass; `node --check` on the extracted script clean; RLS verified live via a role-simulated transaction (`SET LOCAL ROLE authenticated` + `request.jwt.claims`) as Manna's real owner — cross-tenant SELECT against a synthetic "Test Business Two" row returned 0, a cross-tenant INSERT attempt was rejected with `42501`, and same-tenant SELECT/INSERT both succeeded (rolled back, not persisted).
...
## 8ar. Activity Log full rebuild — display + query layer (PR #95, no schema change)

The follow-up PR #95 explicitly asked to build everything §8aq deferred: the name-resolution fix, field-level diffs, filters, real pagination, and parity across the per-client/per-employee views. Pure display/query layer — capture (PR #94) already confirmed complete, so no migration here.

**Part 1 — name resolution, root-caused in §8aq.** `_audClientName`/`_audEmpName` (and a new `_audTeamName`, since `team`-entity rows had no dedicated branch at all before this PR and fell to a generic "actor updated team" fallback) now try the **current row's own snapshot first** (`nv.first_name`/`ov.first_name` — always present for a full-row jsonb dump of that entity's own table), only fall back to the live facade cache, then a **same-batch sibling snapshot** (`_buildAuditContext` now indexes every row's own name into a `ctx.nameHints` map as it walks the batch, so e.g. a job pointing at a since-removed client can still recover the name from that client's own audit rows in the same render), and finally return `null` — never a truthy placeholder — so a genuine miss reads as an honest omission ("Tom Manna updated Job") instead of fabricated-looking text ("(deleted client)"). Also fixed the id-shape bug at its second call site: `openClientActivityLog`'s `direct` query was filtering `entity_id.eq.<external_id>` against a UUID column, always matching zero rows — now uses `c.uuid`.

**Part 2 — field-level diffs.** New `_AUDIT_FIELD_TYPE` (money/time/date/bool/datetime) and `_AUDIT_QUOTED_FIELDS` maps drive a single formatter, `_audFmtDiffSide(field, raw, isTo)`, baked directly into `_audDiffFields`'s output — every caller gets `"$180.00 → $220.00"`, `"9:00 AM → 10:00 AM"`, `"driver: no → yes"`, `"note: (empty) → 'Prefers front door key'"` / `"note: 'Old note' → (removed)"` for free, with no per-caller reformatting left anywhere (removed several call-site-local `new Date(...).toLocaleTimeString` duplicates this uncovered along the way). New `_AUDIT_MEANINGFUL_FIELDS` allowlist (client/job/employee/team/daily_assignment/payment/incident — real column names, checked against `information_schema` for each table, not guessed) scopes diffs to what Tom actually asked to see; entity types without an entry keep the prior blocklist-only behavior unchanged. Creations now list key fields instead of diffing against nothing (`"Tom Manna created Client: Sarah Wilson, 123 Main St, Kingston, WEEKLY, $180.00"`). Client cancellations show the reason/notes plus a future-jobs-cascaded count, computed by a new `_enrichCancellationCascades()` pass that correlates each client-cancelled row against sibling `job`/`cancelled` rows sharing the same `client_id` (via `external_id`, matching how `jobs.client_id` is actually shaped) within a tight time window of the client row's own timestamp — `audit_log` has no explicit "caused by" link between rows, so this is a correlation, not a join; it's the best available signal without adding a new column. Employee `team_id` diffs resolve through `_audTeamName` instead of showing a raw uuid.

**Part 3 — filters.** New filter bar above the Activity Log list: Entity (All/Client/Job/Employee/Team/Assignment/Payment/Chat/Incident/Reward — "Reward" expands to an `.in()` over the three PR #94 reward entity types), Action (the real `action_type` values that actually get written, not a fabricated subset — "Assigned" from Tom's spec isn't its own `action_type`, it's `Entity=Assignment + Action=Created`, so it's reachable via that combination instead of a redundant option; "Moved" is a real value in the CHECK constraint but is never actually written by the trigger today, so it's listed for schema-correctness but will show zero rows until something writes it), Actor (real users for the business, fetched once and cached — **"Claire (AI)" is deliberately not offered**, since §8aq confirmed Claire's tool-driven writes execute through the same authenticated session as the signed-in user with no way to distinguish them today; an option that can't filter anything would be worse than omitting it), Date range (Today/This week/This month/Last 30 days/Custom, `_activityLogDateRangeFor()`), and free-text Search (`old_values::text ilike`/`new_values::text ilike`, debounced). All filters combine via plain chained `.eq()`/`.gte()`/`.lte()` calls (unambiguous AND); the two genuinely-OR filters (search, pagination cursor) each use their own `.or()` call, which PostgREST ANDs together with everything else — verified the underlying SQL semantics directly, but this specific combination (searching while scrolled past page 1) hasn't been exercised in a live browser in this environment.

**Part 4 — pagination + grouping.** Replaced the flat `.limit(250)` / first-8-dates-only render with real server-side cursor pagination: `created_at DESC, id DESC` tiebreak (id isn't meaningfully ordered, it's just a stable second sort key so rows sharing an exact timestamp never get skipped or duplicated across a page boundary — verified live against real data: 5-row page 1 → cursor → 5-row page 2, zero gap/overlap at the boundary). "Load 50 more" button appends to the existing list rather than replacing it. Date-group headers now carry a count (`"Wednesday, Aug 5 — 47 events"`). Empty state reads "No activity matches these filters." per spec.

**Part 5 — per-client/per-employee views get this for free.** `openClientActivityLog`, `_renderClientHistorySection` (client-card inline history), and `_renderStaffActivitySection` (employee-card inline history) all render through the same shared `_renderAuditRowSummary`/`_renderAuditRowHtml`/`_buildAuditContext` functions Parts 1-2 upgraded — no separate rendering code to duplicate-fix. The only surface-specific bug was `openClientActivityLog`'s own query (fixed under Part 1 above); everything else inherited the fix automatically. The Open Items admin view and the employee/client inline history sections pull from native tables (`incidents`/`job_issues`/`client_requests`/`payments`) via a synthetic-row adapter (`_buildSyntheticAuditRow`), not `audit_log` directly — same adapter, same shared renderer, same automatic benefit.

Verified: 28 Node assertions (extraction-based against the real source) covering formatters, the empty/removed wording, the meaningful-fields allowlist (including that an unlisted field is correctly excluded and that entity types with no allowlist entry keep blocklist-only behavior), snapshot-first name resolution across all 4 fallback tiers ending in an honest `null`, and the date-preset math; `node --check` on the extracted script clean; the `::text ilike` search cast verified live to find real matching rows; the cursor-pagination boundary verified live against real `audit_log` data (no gap, no duplicate).

---

## 8as. "Debbie Hoffman's jobs didn't populate" — a two-part investigation, one real bug (PR #96)

Tom reported Debbie Hoffman (then Amanda Kane too, mid-investigation) as a regular recurring client whose schedule "did NOT populate" after the Maids CSV import, suspecting silent row-skipping. Direct SQL against `jobs` told a different story before Tom's follow-up reframed the question: Hoffman had 27 job rows (22 completed spanning 2025-01-07 → 2026-07-21 on a consistent ~28-day cadence, plus 5 future `scheduled` rows through Dec 2026); Kane had 30, same pattern. Both fully populated, nothing missing. Tom then clarified what he was actually seeing: a **"cust" badge** on her client card, which he read as an OMS/one-time marker contradicting "recurring" — i.e., the perceived "didn't populate" was really "this card looks OMS-categorized, so I assumed it had no real schedule," not an actual data gap.

**Item 1 — badge rename.** `FREQ_SHORT['OMS']` (index.html, the Clients-tab card's `freq-chip`) mapped to `'CUST'`. A **second, independent** frequency-badge implementation on the Schedule board (`frequencyBadge()`, ~line 40447) already correctly showed `'OMS'` for the identical case — another instance of the mirrored-copy-diverges pattern this session keeps finding, just cosmetic this time. Changed `FREQ_SHORT['OMS']` to `'OMS'` to match the already-correct copy.

**Item 2 — the real bug, one level deeper than the badge.** Debbie Hoffman's `tags` array correctly read `['recurring']` (not `oms_only`) — the tag logic, which gates the Recurring tab's own filter (`ctags.includes('oms_only')`), was never wrong. But her `frequency` DB column read literally `'OMS'` — a **different field, driven by a different bug**, and it's `frequency` (via `_normalizeFreqCode` → `c.fc`) that the badge reads, not `tags`. Root cause, found in `maidsBuildPreview`: a client's `rows_frequency` is taken from **whichever row is chronologically latest across all 3 uploaded CSVs**, with no regard for which file it came from. The 2025_combined file "mixes both categories" (recurring and OMS rows together) per the code's own existing comment — so a recurring client with a one-off OMS-flagged add-on visit logged in that file, dated later than their real 2026_recurring-file cadence, had their `frequency` silently overwritten to `'OMS'` even though their `tags` (computed via simple file-membership, `hasRecurring2026`) stayed correct the whole time. Two independently-computed signals (`tags` vs `frequency`) that are supposed to agree, quietly drifting apart — same failure shape as the badge duplication above, one layer down.

**Swept the DB** for `tags @> ['recurring'] AND frequency = 'OMS'` (a precise, deterministic proxy for this exact bug, confirmed against the code mechanism rather than guessed): **13 clients**, including Hoffman and Kane. **Fix, forward-looking**: `maidsBuildPreview` now excludes OMS-categorized rows from the latest-row frequency pick whenever the client has any `2026_recurring` row at all (mirrors the same signal the tag logic already uses), falling back to the original all-rows pick only in the (structurally impossible today) case where every row is somehow OMS-categorized despite that.

**Backfill, migration 096**: of the 13, **6 had enough real job history to infer their correct cadence with confidence** (≥5 completed/scheduled jobs, consistent median gap between visits, mapped to the nearest standard code) — Dilworth/Hoffman/Kane/Myer → `RMS-MON` (median gap ~28-30d), Doucette → `RMS-EOW` (14d), Gallagher → `RMS-ETW` (21d). Confirmed live post-backfill: correct `frequency`, `tags` unchanged, `audit_log` captured all 6 as `action_type='updated'` with `user_id=NULL` (System actor — accurate, this was a migration, not a manual edit). The other 7 (Shannon DelGallo, Mari Donahue, Rebecca Duong, Jennifer Garner-DNC, Jennifer Heaton, Odelle Kinder-Wells, Ellen Ziedenburg) have 0-2 jobs on file — not enough signal to infer a cadence reliably, and the source CSVs aren't available to check directly (client-side-only import, confirmed in PR #94's capture sweep — never persisted server-side). Left as `frequency='OMS'` and reported to Tom by name rather than guessed.

**Related, separate finding from the same investigation — fixed in PR #97, see §8at**: `client_external_id` for a group's job rows was resolved independently of `maidsMatchClient`'s own match result, orphaning jobs whenever a matched client's real external_id differed from the CSV group's own key.

**Also confirmed, not a gap at the time — wired up in PR #97, see §8at**: `import_runs.warnings_json` existed as a column but was never populated.

---

## 8at. Orphaned import job repair + warnings persistence (PR #97, migration 097)

The two items §8as flagged but didn't fix.

**Item A — root cause and prevention.** In `maidsBuildPreview`, a CSV row-group's jobs got `client_external_id` from `groupExternalId` (the group's own first-row `customerid`, or a fabricated `name|street` key when absent) — computed **completely independently** of `maidsMatchClient`'s actual match result. Meanwhile `maidsBuildPayload`'s `pushUpdate(entry)` correctly sets a matched client's `external_id` to `entry.match.client.id` (their real, pre-existing id). The original code's own comment claimed these two would "always line up post-commit" — false whenever a matched client's rows are inconsistent about carrying a `customerid` (their group's key ends up being name+street instead, which is a different string than their real external_id). The client record updated fine; that client's *jobs* silently attached to nothing.

Fix: `maidsBuildPayload` now builds one `keyToExternalId` map, populated **inline inside `pushUpdate`/`pushCreate` themselves** (the only two places that decide a client's real external_id) rather than a second, separately-computed copy of the same tier/decision branching — avoids exactly the "mirrored algorithm copies diverge" failure class this session keeps finding. Each job now carries a `group_key` (added in `maidsBuildPreview`) and resolves its final `client_external_id` through that map in `maidsBuildPayload`, with a defensive fallback to the old value (should never trigger — every job's group always goes through `pushUpdate`/`pushCreate`). Verified behaviorally (not just by source inspection): extracted the real `maidsBuildPayload` and ran it against synthetic preview data reproducing the exact bug shape (a matched client whose group key differs from their real external_id) — job now resolves to the client's real id; a genuinely new client's jobs still resolve to their own key (that case was never broken); a confirm-tier row Tom rejects as a match still uses its own key, not the rejected candidate's id.

**Item A — repair.** 8 orphaned job rows across 4 client_ids (10851645, 10875330, 10906649, 10956520 — none matching any existing client, confirmed via a trimmed-comparison check to rule out a formatting fluke) got a client row each, created via migration 097: `first_name='Unidentified'`, `last_name='Maids #<id>'`, `status='active'` (one of the four — 10956520 — has a job scheduled for the day the migration ran, so hiding it as inactive would have hidden a live appointment), `review_flags=['unidentified_import_customer']`, and a `notes` field summarizing each client's actual job history (dates/prices/teams) so Tom can identify and merge/rename them if he recognizes the pattern. The source CSVs aren't stored server-side (confirmed in §8aq/PR #94's capture sweep), so there was no way to recover their real name/address — the stubs are explicitly flagged rather than guessed onto a real client, which risked corrupting someone else's history. Their derived stats (`historical_completion_count`, `first_service_date`, etc. — never computed for them since they didn't exist during the original import's recompute pass) were backfilled in the same migration using the identical aggregation `import_maids_finish` itself runs. Verified live: 0 orphaned job rows remain post-migration.

**Item B.** `import_maids_finish` gained a `p_warnings jsonb DEFAULT '[]'::jsonb` parameter, written straight to `import_runs.warnings_json` (previously declared, never touched). `maidsRunCommitBatched`'s finish call now passes `_maidsState.preview.warnings` — already fully computed by `maidsBuildPreview` (summary-row skips, empty-date skips, unmapped team codes, status anomalies), just never sent anywhere before. One deploy wrinkle worth flagging for future signature changes: `CREATE OR REPLACE FUNCTION` does **not** replace a function whose parameter list changed — even just appending a defaulted parameter creates a second, distinct overload in Postgres rather than replacing the first, which briefly left two `import_maids_finish` functions live (a real risk of a PostgREST "could not choose the best candidate function" ambiguity error). Caught immediately via `pg_proc`, fixed with an explicit `DROP FUNCTION` on the old 6-arg signature before the `CREATE OR REPLACE`, now folded into the migration file itself so a fresh environment replaying migrations in order lands in the same clean single-function state.

Verified: 8 Node assertions, 5 of them running the real extracted `maidsBuildPayload` (not a hand-reimplementation) against synthetic data reproducing the actual bug shape; `node --check` clean; live rolled-back RPC call confirmed `p_warnings` persists to `warnings_json` correctly; live anti-join confirmed zero orphaned job rows remain.

---

## 8au. CSV audit staging — compare Maids exports to Penta without importing (PR #98, migration 098)

A prior turn's DB-only audit (comparing `tags`-as-recurring-membership-proxy against Penta's current client list) produced a report Tom then reacted to with names and an "Item 2" list that, on direct re-read of the actual conversation, never appeared anywhere in what was sent — confirmed by grepping the real prior message for the disputed names and finding none of them. Likely a different report/session got attributed to this one; not chased further since it wasn't reproducible here. Root issue underneath both turns was the same, though: no way to do a REAL CSV-vs-Penta comparison, because (per §8aq/PR #94's capture-completeness sweep) the 3 Maids CSVs are read entirely client-side and never persisted server-side — nothing to query them against after the fact.

**Fix, scoped exactly to Tom's ask**: a new audit-only path through the existing Maids Import modal. `maidsStageForAudit()` takes whatever CSVs are currently loaded into `_maidsState.csvRows` (the same client-side parse `maidsRunPreview` already uses) and inserts the **raw, unprocessed rows** into a new `maids_import_staging` table — no `maidsMatchClient` fuzzy scoring, no client/job writes, no dedup, no multi-visit handling, none of `maidsBuildPreview`'s logic runs at all. A new "🔍 Stage for Audit (no import — creates or changes nothing)" button sits below the existing "Preview Import" button, batched the same way job commits are (500 rows/batch, same statement-timeout rationale as migration 089). Each staging run gets its own `run_id` so a re-upload doesn't collide with a prior one.

One deliberate behavioral difference from a real import, worth knowing when reading staged data: **rows with an empty `date` are still staged** (as `raw_date: null`), where `maidsBuildPreview` would silently skip them entirely (`emptyDateSkipCount++`). Staging is meant to show ground truth, not the import pipeline's own filtering — verified with a Node test asserting exactly this (a row with `date: ''` lands in the staging payload, not dropped).

`maids_import_staging`: `business_id`, `run_id`, `source_type` (CHECK'd to the same 3 values `MAIDS_CSV_LABELS` uses), `customer_id`/`raw_name`/`raw_address`/`raw_date`/`raw_frequency` (extracted for fast SQL filtering) plus a `raw jsonb` column carrying the complete original parsed row (nothing dropped, so any other CSV column is still recoverable later). RLS: SELECT/INSERT/DELETE only (no UPDATE — staged rows are write-once, deleted when the audit's done, never edited in place), same inline-subquery tenant-scoping pattern as every other table this session. Verified live: same-tenant insert+select works, a cross-tenant insert attempt was rejected with `42501`, both rolled back.

Explicitly temporary, per Tom's own framing — no automatic cleanup job. Staged rows should be deleted once the comparison report is delivered (either by Tom from the app, or a follow-up one-off `DELETE ... WHERE run_id = ...`).

**Investigated the 10 "no future scheduled visit" clients from the prior turn using current DB state** (doesn't need the CSV): none are paused (`pause_start`/`pause_end` both null for all 10), none are cancelled or soft-deleted, all `status='active'`. Several (Fitchburg State University, Ellen Hazen, Amanda Henderson, Jean White) have a `status_changed_at` of `2026-04-26`, predating the `2026-08-05` wipe-mode import entirely — confirming they're genuine longstanding Penta clients (matched and updated by the import, not freshly created by it), not junk data. Every one of the 10's most recent job (regardless of status) falls somewhere between January and July 2026 — none have a job dated anywhere near "today" or beyond, unlike Hoffman/Kane's group which had rows running into December. Genuinely ambiguous without the source CSV whether this means (a) Maids' own export simply hadn't scheduled these 10 out any further at the time of that export, or (b) something failed to carry a real future row through — the staging table above is exactly what resolves this once Tom re-uploads.
...
- **098** — CSV audit staging (§8au). New `maids_import_staging` table for the audit-only "Stage for Audit" path in the Maids Import modal — raw CSV rows only, no client/job writes. SELECT/INSERT/DELETE RLS, no UPDATE (write-once, deleted when done). Explicitly temporary per Tom's own framing; no auto-cleanup.
- **097** — Orphaned import job repair + warnings persistence (§8at). Creates 4 flagged placeholder clients to attach 8 previously-orphaned job rows (root cause fixed in `index.html`'s `maidsBuildPayload`); backfills their derived stats; drops the now-superseded 6-arg `import_maids_finish` overload and replaces it with a 7-arg version that persists `p_warnings` to `import_runs.warnings_json`.
- **096** — Backfill mislabeled OMS frequency (§8as). One-time data fix, no schema change: 6 clients (Dilworth, Doucette, Gallagher, Hoffman, Kane, Myer) had `frequency='OMS'` despite being genuinely recurring (`tags` already correctly said so) — backfilled to their real cadence, inferred from actual job-visit intervals. 7 more clients with the same tag/frequency mismatch but too little job history to infer confidently were left as-is and reported by name instead of guessed.
