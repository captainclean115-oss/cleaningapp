...
| `geotab-call` (v1, feat/geotab-server-side) | JWT required | Server-side proxy for MyGeotab's JSON-RPC API (PR1 of Geotab strategy split, mig 067). Reads credentials from per-tenant `business_geotab_integrations`. POST `{method, params}`. Method allowlist: `Get`, `GetCountOf` (read-only; write methods deferred). Authenticates server-side, caches the session (including the post-pivot host returned in `result.path`) per `business_id` in-memory ~50min, retries once on session-expired. Browser's 8+ `geotabCall(...)` sites still use the hardcoded constants — PR2 will rewire them through `_geotabApi(method, params)` |
