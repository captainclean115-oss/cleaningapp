-- v11.0.25 — Extend audit_log for form_submissions.
--
-- The forms table (Sprint 8) was capturing employee form submissions
-- but wasn't wired into the master audit_log. This brings forms into
-- line with the Phase A/B/C/D entities so:
--   - Activity Log surfaces "Viviana submitted Time Off form" rows
--   - Manager profile view of employee shows form history through the
--     same _buildSyntheticAuditRow adapter
--   - Updates tab badge / inbox can count pending forms via audit_log
--     queries (alongside the direct PentaForms read).
--
-- Three pieces:
--   1. Add 'form_submission' to entity_type CHECK
--   2. Extend audit_log_capture() to handle forms:
--      - INSERT  → action_type='submitted'  (new vocab value)
--      - UPDATE  status pending → approved → 'approved'
--      - UPDATE  status pending → denied   → 'rejected'
--      - Other UPDATE → 'updated'
--   3. Attach trigger to public.forms
...
ALTER TABLE public.audit_log
  ADD CONSTRAINT audit_log_entity_type_chk CHECK (entity_type IN (
    ...
    'client_request','chat_message','form_submission'
  ));

-- 'submitted' is already in the action_type CHECK (mig 042).
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    ELSIF TG_TABLE_NAME = 'forms' THEN
      v_action := 'submitted';  -- v11.0.25: form submission
    ELSE
      v_action := 'created';
    END IF;
  ...
  ELSE
    ...
    -- v11.0.25: forms status transitions
...
DROP TRIGGER IF EXISTS audit_forms_capture ON public.forms;
CREATE TRIGGER audit_forms_capture
