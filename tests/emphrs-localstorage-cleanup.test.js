...
// overrides to Supabase (PR #171). On a browser that had accumulated
// many of these over time, they were the confirmed direct cause of the
// "hitting quota limits" report -- getEmpHours/saveEmpHours/
// clearEmpHours no longer read or write this key shape at all, so
// anything still there is pure dead weight.
//
...
const startMarker = "// PR #172 -- one-time cleanup.";
const startIdx = src.indexOf(startMarker);
if (startIdx === -1) { console.error('FAIL: could not find the PR #172 cleanup block'); process.exit(1); }
const endMarker = "\n})();\n\n// PR #119's cleanup";
...
// A browser with several stale overrides plus unrelated keys that must survive.
{
  ...
  check('both stale empHrs_* keys are removed', Object.keys(localStorage._store).filter(k => k.indexOf('empHrs_') === 0).length, 0);
  ...
  check('a removal count was logged', logs.some(l => /removed 2 stale/.test(l)), true);
}
...
// A browser with no stale keys at all -- must not log a spurious removal count.
{
  const { localStorage, logs } = run({ 'weekHours_cache': '{}' });
  check('flag is still set even when nothing needed removing', localStorage.getItem('_pr172_emphrs_localstorage_cleanup_done'), '1');
  check('nothing is logged when there was nothing to remove', logs.length, 0);
}
